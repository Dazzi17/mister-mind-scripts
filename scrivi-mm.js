...
(function injectMisterMindStyles() {
  ...
  s.textContent = "@import url('https://fonts.googleapis.com/css2?family=Rubik:wght@400;500;600&family=Fraunces:wght@600;700&display=swap');" +
...
(function () {
    ...
    const consultationUrl = 'https://lugano.adi/';
    ...
    function buildUrlParams() {
        ...
        if (config.termsUrl) { urlParams.set('termsUrl', config.termsUrl); urlParams.set('termsText', config.termsText); }
        if (config.privacyUrl) { urlParams.set('privacyUrl', config.privacyUrl); urlParams.set('privacyText', config.privacyText); }
    ...
    if (urlParams) aiChatUrl = aiChatUrl + '&' + urlParams;
    ...
    .original_overlay {
        ...
        background-color:rgba(0,0,0,0.5); z-index:10002; opacity:0; transition:opacity 0.3s ease-out;
    }
    .original_overlay.original_show { display:block; animation:fadeIn 0.3s ease-out forwards; }
    .original_overlay.original_hide { animation:fadeOut 0.3s ease-out forwards; }

    .original_popup {
        display:none; position:fixed; top:0; left:0; transform:none;
        width:95vw; height:95vh; background-color:white; border-radius:10px;
        box-shadow:0 0 20px rgba(0,0,0,0.2); z-index:10003; opacity:0; transition:opacity 0.3s ease-out;
    }
    .original_popup.original_show { display:flex; animation:fadeIn 0.3s ease-out forwards; }
    .original_popup.original_hide { animation:fadeOut 0.3s ease-out forwards; }
    .original_popup-content { position:relative; width:100%; height:100%; display:flex; }

    .original_sidebar {
        width:380px; min-width:380px; background-color:#171717;
        box-shadow:2px 0 5px rgba(0,0,0,0.1); padding:40px 60px 20px 60px;
        display:flex; flex-direction:column; border-radius:10px 0 0 10px;
        max-height:95vh; overflow-y:auto; gap:20px;
    }
    .original_sidebar-content { display:flex; flex-direction:column; gap:20px; flex:1; }
    .original_sidebar-main { flex:1; }
    .original_sidebar-title { color:#f6f5f5; font-weight:800; font-size:19px; margin-bottom:24px; line-height:24px; }
    .original_sidebar-widgets { display:flex; flex-direction:column; gap:24px; }
    .original_sidebar-widget { text-decoration:none; color:inherit; transition:background-color 0.3s ease; }
    .original_sidebar-widget.original_box, .original_sidebar-widget.original_adia { padding:16px; background-color:#454545; text-decoration:none; }
    .original_sidebar-widget.original_adia.original_offline { cursor:not-allowed; opacity:0.5; }
    .original_sidebar-widget.original_link { color:#f6f5f5; font-weight:bold; font-size:16px; display:flex; align-items:center; gap:16px; }
    .original_sidebar-widget.original_link:hover { text-decoration:underline; }
    .original_widget-header { display:flex; align-items:center; justify-content:space-between; margin-bottom:12px; }
    .original_widget-title { font-weight:800; font-size:16px; text-decoration:underline; color:#f6f5f5; }
    .original_widget-description { color:#f6f5f5; font-size:14px; line-height:1.4; margin-bottom:0; }
    .original_widget-arrow { width:12px; height:12px; fill:#f6f5f5; }
    .original_widget-status { display:flex; justify-content:end; align-items:center; gap:8px; margin-top:12px; }
    ...
    .original_sidebar-bottom { background-color:#f6f5f5; padding:16px; flex-shrink:0; margin-top:auto; }
    .original_sidebar-bottom-title { color:#171717; font-weight:800; font-size:16px; margin-bottom:12px; }
    .original_sidebar-bottom-description { color:#171717; font-size:14px; line-height:1.4; }

    .original_chat-container { flex:1; position:relative; overflow:hidden; border-radius:0 10px 10px 0; }
    .original_close-button { position:absolute; top:10px; right:22px; font-size:24px; cursor:pointer; background:none; border:none; color:#333; z-index:1; }
    .original_chat-iframe { width:100%; height:100%; display:block; border:none; border-radius:0; }
    .original_loader {
        display:none; position:absolute; top:50%; left:50%;
        transform:translate(-50%,-50%); width:50px; height:50px;
        border:5px solid #f3f3f3; border-top:5px solid #bf0d3e;
        border-radius:50%; animation:spin 1s linear infinite;
    }

    .original_confirmation-modal {
        display:none; position:fixed; top:50%; left:50%; transform:translate(-50%,-50%);
        width:400px; max-width:90%; background-color:white; border-radius:10px;
        box-shadow:0 0 20px rgba(0,0,0,0.2); z-index:10004; padding:30px; text-align:center;
        opacity:0; transition:opacity 0.3s ease-out;
    }
    .original_confirmation-modal.original_show { display:block; animation:fadeIn 0.3s ease-out forwards; }
    .original_confirmation-modal.original_hide { animation:fadeOut 0.3s ease-out forwards; }
    .original_confirmation-title { font-size:24px; margin-bottom:30px; font-weight:bold; }
    .original_confirmation-buttons { display:flex; flex-direction:column; gap:15px; }
    .original_confirmation-button { padding:15px; border-radius:50px; font-size:16px; font-weight:bold; cursor:pointer; border:none; width:100%; }
    ...
    .original_feedback-modal {
        display:none; position:fixed; top:50%; left:50%; transform:translate(-50%,-50%);
        width:500px; max-width:90%; background-color:white; border-radius:10px;
        box-shadow:0 0 20px rgba(0,0,0,0.2); z-index:10005; padding:40px; text-align:left;
        opacity:0; transition:opacity 0.3s ease-out;
    }
    .original_feedback-modal.original_show { display:block; animation:fadeIn 0.3s ease-out forwards; }
    .original_feedback-modal.original_hide { animation:fadeOut 0.3s ease-out forwards; }
    .original_feedback-title { font-size:28px; margin-bottom:10px; font-weight:bold; color:#333; line-height:1.3; }
    .original_feedback-subtitle { font-size:18px; margin-bottom:30px; color:#666; line-height:1.5; }
    .original_feedback-rating-title { font-size:18px; font-weight:bold; margin-bottom:15px; color:#333; }
    .original_star-rating-container { display:flex; justify-content:space-between; align-items:center; margin-bottom:30px; }
    .original_star-rating { display:flex; gap:5px; }
    .original_star { font-size:30px; cursor:pointer; color:#e0e0e0; transition:color 0.2s ease; }
    .original_star.original_active { color:#000; }
    .original_feedback-details-label { font-size:18px; font-weight:bold; margin-bottom:15px; }
    .original_feedback-textarea { width:100%; height:200px; padding:15px; border:1px solid #ddd; border-radius:5px; resize:none; font-size:16px; margin-bottom:30px; font-family:inherit; }
    .original_feedback-buttons { display:flex; flex-direction:column; gap:15px; }
    .original_feedback-submit { background-color:#000; color:white; padding:15px; border-radius:50px; font-size:16px; font-weight:bold; cursor:pointer; border:none; width:100%; transition:background-color 0.3s ease; }
    .original_feedback-submit:disabled { background-color:#ccc; cursor:not-allowed; }
    .original_feedback-skip { background-color:transparent; color:#666; padding:15px; font-size:16px; cursor:pointer; border:none; text-align:center; text-decoration:underline; }

    @media(max-width:768px) {
        .original_popup { width:100vw; height:calc(var(--vh, 1vh) * 100); border-radius:0; top:0; left:0; transform:none; }
        .original_popup-content { flex-direction:column; }
        .original_sidebar { width:100%; min-width:unset; border-radius:0; padding:60px 20px 10px 20px; display:none; flex:1; height:calc((var(--vh, 1vh) * 100) - 60px); max-height:calc((var(--vh, 1vh) * 100) - 60px); overflow-y:auto; gap:15px; }
        ...
        .original_chat-container { flex:1; display:flex; flex-direction:column; height:calc((var(--vh, 1vh) * 100) - 60px); border-radius:0; }
        .original_chat-container.original_mobile-hidden { display:none; }
        .original_chat-iframe { border-radius:0; flex:1; }
        .original_mobile-nav { display:flex; background-color:#171717; border-top:1px solid #454545; height:60px; position:relative; z-index:1; order:2; }
        .original_mobile-nav-button { flex:1; background:#171717; border:none; padding:15px; font-size:16px; font-weight:600; cursor:pointer; color:#f6f5f5; transition:all 0.3s ease; display:flex; align-items:center; justify-content:center; gap:8px; }
        ...
        .original_close-button { position:fixed; top:15px; right:15px; z-index:1000; background-color:rgba(255,255,255,0.9); border-radius:50%; width:40px; height:40px; display:flex; align-items:center; justify-content:center; box-shadow:0 2px 8px rgba(0,0,0,0.2); }
    }
    ...
    @media(prefers-color-scheme:light) {
        ...
        .original_confirmation-modal { background-color:#FFFFFF; }
        ...
        .original_feedback-modal { background-color:#FFFFFF; }
    ...
    body.original_chat-widget-open { overflow:hidden!important; overscroll-behavior:contain; }
`;
...
<a href="javascript:void(0)" class="original_sidebar-widget original_box" data-widget-index="${index}">
    <div class="original_widget-header">
        ...
        <svg class="original_widget-arrow" viewBox="0 0 8 12" xmlns="http://www.w3.org/2000/svg"><path d="M0.59 10.59L5.17 6L0.59 1.41L2 0L8 6L2 12L0.59 10.59Z"/></svg>
    </div>
...
<div class="original_sidebar">
    <div class="original_sidebar-content">
        <div class="original_sidebar-main">
            ...
            <div class="original_sidebar-widgets">${widgetsHTML}</div>
        </div>
    ...
    // Nessun bottone laterale – il trigger è #scrivi-mm già nel sito
...
<div id="original_feedback-modal" class="original_feedback-modal">
    ...
    class ChatWidget {
        ...
        cacheElements() {
            const elementIds = [
                'original_chat-iframe', 'original_overlay', 'original_popup', 'original_close-button',
        ...
        attachEventListeners() {
            // Bottone del sito Mister Mind – con fallback DOMContentLoaded
            const attachExternalButton = () => {
            ...
            if (document.readyState === 'loading') {
                ...
                attachExternalButton();
            }

            if (this.elements.overlay) this.elements.overlay.addEventListener('click', () => this.showConfirmation());
            if (this.elements.closebutton) this.elements.closebutton.addEventListener('click', () => this.showConfirmation());
            if (this.elements.confirmclose) this.elements.confirmclose.addEventListener('click', () => this.showFeedback());
            if (this.elements.cancelclose) this.elements.cancelclose.addEventListener('click', () => this.cancelClose());
            if (this.elements.feedbacksubmit) this.elements.feedbacksubmit.addEventListener('click', () => this.submitFeedback());
            if (this.elements.feedbackskip) this.elements.feedbackskip.addEventListener('click', () => this.skipFeedback());
            ...
            if (this.elements.mobilenavchatbot) this.elements.mobilenavchatbot.addEventListener('click', () => this.switchMobileView('chatbot'));
            if (this.elements.mobilenavsupport) this.elements.mobilenavsupport.addEventListener('click', () => this.switchMobileView('support'));
        ...
        async checkAdiaStatus(widgetIndex) {
            ...
            try {
                const response = await fetch('https://.adia./serviceinfo/lugano');
        ...
        updateAdiaWidget(widgetElement, widgetIndex, isOnline) {
            ...
            if (isOnline) {
                ...
                statusIcon.classList.replace('original_offline', 'original_online');
                statusText.classList.replace('original_offline', 'original_online');
                statusText.textContent = 'Online';
                widgetTitle.classList.replace('original_offline', 'original_online');
                widgetElement.title = widget.onlineText || 'Consultant available';
            } else {
                widgetElement.classList.add('original_offline');
                statusIcon.classList.replace('original_online', 'original_offline');
                statusText.classList.replace('original_online', 'original_offline');
                statusText.textContent = 'Offline';
                widgetTitle.classList.replace('original_online', 'original_offline');
        ...
        handleSidebarWidgetClick(event) {
            ...
            if (widget.target === 'internal') { this.openIframe(consultationUrl); }
            else if (widget.target === '_blank') { window.open(widget.url, '_blank'); }
            else { window.location.href = widget.url; }
        }
        ...
        showFeedback() {
            this.hideModal(this.elements.confirmationmodal, null, () => {
                if (!this.conversationId) { this.closePopup(); return; }
